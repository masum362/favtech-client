...
const UpdateProduct = () => {
    ...
    const handleFormSubmit = async (data) => {
        console.log(data);
        const response = await authSecure.patch(`/update-product/${product._id}`, { product: data })
        try {
    ...
    return (
        <div>
            <h1 className='text-2xl md:text-5xl font-bold text-center'>Update Product</h1>
            <div className='flex flex-col items-center justify-center lg:my-12' >
                ...
                <div className='max-w-xl w-full bg-slate-50 p-4 shadow-lg rounded-lg' >
                    <form className='w-full space-y-4 my-8 ' onSubmit={handleSubmit(handleFormSubmit)}>
                        ...
                        {!loading ? <CustomBtn btnType={"submit"} text={"Update"} style={"w-full"} /> : <div className='text-themePrimary'>
